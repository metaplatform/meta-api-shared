...
module.exports = function(ws, logger, connConstructor, authHandler){
	...
	var respond = function(rid, data, command){
		...
		logger.debug("Sending response to %s, RID: %s, Command: %s", ws.upgradeReq.connection.remoteAddress, rid, res.c, res.t);
	 ...
	 */
	 ...
	 var HandleResponse = function(rid, err, data, type){
	 ...
	 Auth = function(params){
	 	...
	 	return new Promise(function(resolve, reject){
	 		...
	 		try {
	 			...
	 			connConstructor(params.serviceName, params, function(endpoint, method, params){
		...
		try {
			...
			switch(data.c){
				...
				case protocol.commands.response:
					if(data.r)
						HandleResponse(data.r, null, data.d, data.t);
					return;
