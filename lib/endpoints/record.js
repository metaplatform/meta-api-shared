...
var Record = function(schema){
	...
	var Endpoint = function(parent, id){
		...
		this._gcInterval = schema.gcInterval || 10000;
		...
		setInterval(function(){
			...
			if(!self._liveHandler) return;

			var now = (new Date()).getTime();

			if(self._liveHandler.updated + 10000 >= now) return;

			self._liveHandler.api.subscribers(self._liveHandler.channel.toString()).then(function(cnt){

				if(cnt === 0){
					console.log("*COLLECT");
					self._liveHandler.ref.unsubscribe();
					self._liveHandler = null;
				}
	...
	if(schema.live) Endpoint.prototype["@live"] = Record.Method({
		...
		if(this._liveHandler){
			...
			this._liveHandler.updated = (new Date().getTime());
			return Promise.resolve(this._liveHandler.channel);
			...
			return schema.live.call(this).then(function(ref){
				...
				return self._liveHandler.channel;
	...
	Endpoint.prototype.emitLive = function(op, data){
		...
		if(!this._liveHandler) return;

		var record = data.content;
		record["@rid"] = "#" + data.cluster + ":" + data.position;

		console.log("LIVE RECORD", data);

		this._liveHandler.api.publish(this._liveHandler.channel.toString(), {
			op: op,
			record: record
		}).then(function(cnt){
