		...
		try {
			...
			var getEndpoint = function(dirName){
				...
				return function(parentEndpoint){
					...
					if(!r.then || !(r.then instanceof Function))
 ...
 * @resolve Object
 */
Client.prototype.callUri = function(uri, method, params){

	var parts = uri.split("://");

	if(parts.length !== 2) return Promise.reject(new Error("Invalid URI"));

	return this.call(parts[0], parts[1], method, params);
...
Client.prototype.subscribe = function(channel, cb){
	...
	return new Promise(function(resolve, reject){
		...
		try {
			...
			self.connection.subscribe(channel).then(function(){
				...
				resolve({
					unsubscribe: function(){
						return self.unsubscribe(channel, cb);
					}
				});
...
Client.prototype.unsubscribe = function(channel, cb){
	...
	return new Promise(function(resolve, reject){
		...
		try {
			...
			self.subscriptions[channel].splice(i, 1);
			...
			if(self.subscriptions[channel].length === 0){
				...
				self.connection.unsubscribe(channel).then(function(res){
...
Client.prototype.subscribers = function(channel){
	...
	return new Promise(function(resolve, reject){
		...
		try {
			...
			self.connection.subscribers(channel).then(resolve, reject);
	...
	return new Promise(function(resolve, reject){
		...
		try {
			...
			self.connection.subscribeQueue(queue).then(function(){
				...
				return resolve({
					unsubscribe: function(){
						return self.unsubscribeQueue(queue);
					}
				});
...
Client.prototype.unsubscribeQueue = function(queue){
	...
	return new Promise(function(resolve, reject){
		...
		try {
			...
			self.connection.unsubscribeQueue(queue).then(function(res){
