...
/*
 * Validates parameters only present in prams
 *
 * @param Object params
 * @param Boolean strict If to strip properties that are not defined by validators
 * @param String subName Prefix for fields name - for sub-validating
 ...
 * @resolve Object Validated and transformed params
 */
Validator.prototype.validateOptional = function(params, strict, subName){
	...
	for(var i in self)
		if(self[i].validate && self[i].validate instanceof Function && params[i] !== undefined)
			p = p.then(validateParam(i));
 ...
 * @return Array|null
 */
var ObjectArrayValidator = function(opts){

	if(!opts) opts = {};

	BaseValidator.call(this, opts);

	this.type = this.type + ":Array:Object";
	this.properties = ( opts.properties ? new Validator(opts.properties) : null );

};

ObjectArrayValidator.prototype = Object.create(BaseValidator.prototype);

ObjectArrayValidator.prototype.validate = function(name, value, strict){
	...
	if(!value || !this.properties) return [];
	...
	for(var i = 0; i < value.length; i++){
		...
		if(value[i] instanceof Object)
			checks.push(this.properties.validate(value[i], strict, name + "." + i));
		else
			return Promise.reject(new InvalidValueError("NotObject", name, "Array element #" + i + " must be an object."));
...
var LinkValidator = function(opts, apiClient){
	...
	this.pattern = opts.pattern || null;
	this.noValidate = opts.noValidate || false;
...
LinkValidator.prototype.validate = function(name, value, strict){
	...
	if(this.noValidate) return value;

	return this.getClient().callUri(value, "schema").then(function(){
		return value;
	}, function(err){
		throw new InvalidValueError("InvalidLink", name, "Provided API endpoint '" + value + "' is not accessible.");
	});
...
MultiLinkValidator.prototype.validate = function(name, value, strict){
	...
	for(var i in value){
		...
		if(!this.noValidate)
			checks.push(this.getClient().callUri(value[i], "schema"));
 ...
 * @return String|null
 */
var MACValidator = function(opts){

	if(!opts) opts = {};

	BaseValidator.call(this, opts);

	this.type = this.type + ":MAC";
	this.pattern = opts.pattern || null;

};

MACValidator.prototype = Object.create(BaseValidator.prototype);

MACValidator.prototype.validate = function(name, value){
	...
	//Check format
	if(!String(value).match(/^([0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2}$/))
		throw new InvalidValueError("NotIPv4", name, "Value must be a valid MAC address.");
...
module.exports = {
	...
	any: 			function(opts){ return new BaseValidator(opts); },
	...
	objectArray: 	function(opts){ return new ObjectArrayValidator(opts); },
	...
	mac: 	 		function(opts){ return new MACValidator(opts); }
